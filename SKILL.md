...
## 💡 核心原则：AI 文档维护 3 大黄金法则

AI 在执行本技能的任何模式时，必须严格遵守以下 3 大黄金法则：

1. **“同 PR 原则 (Same PR Principle)”**：
   - 严禁代码已合入主干而文档还未更新。代码改动、测试用例与相关 Spec/ADR/Glossary 必须在同一个 Git Commit / PR 中同步交付。
2. **“Token 预算控制 (Keep Rules Lean)”**：
   - `AGENTS.md` 是高频加载的全局上下文，切忌写成大百科全书。只保留红线规则、命令和核心约定（控制在 100 行内），详细的业务逻辑分流到 `docs/specs/` 中按需读取。
3. **“把文档当成 AI 的上下文缓存 (Docs as External Memory)”**：
