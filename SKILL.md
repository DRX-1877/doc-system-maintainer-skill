...
description: AI 编程核心文档体系管家。负责在项目中自适应识别架构、初始化、全生命周期维护并巡检 6 类核心文档（Root/Scoped AI Rules, Specs, ADRs, Business Models & Glossary, Tasks, Runbook），保障 Open-SWE 规范注入与 ATDD 闭环。
---

# 文档体系生命周期管家指令 (doc-system-maintainer)

本技能用于为软件工程项目提供自适应架构感知、从零初始化、全生命周期演进与一致性巡检的 6 大核心文档体系支持，原生兼容 **Open-SWE / Deep Agents 级联规范注入标准**，并提供 **Spec-Driven 即时生成（JIT）的 CI/Pre-commit 物理硬门禁**。
...
2. **“架构自适应与分层级联 (Architecture-Agnostic & Precedence Hierarchy)”**：
   - **架构自适应**：AI 必须首先逆向识别项目的真实架构风格（DDD/六边形架构、传统 MVC、FastAPI/SQLModel 数据驱动、Go Clean Arch 等），**严禁死板套用单一模式**。
   - **分层级联注入**：全局规则置于根目录 `AGENTS.md`，根据识别到的实际分层（如 `domain/`、`service/`、`routers/`、`dao/` 等）下沉局部 `AGENTS.md`。
...
1. **逆向扫描项目特征与架构风格感知**：
   ...
   - 扫描目录与代码，识别实际架构风格（如：DDD 六边形架构、MVC 三层架构、FastAPI 路由与模型、Go 扁平包结构等）。
2. **基于 templates/ 渲染并生成分层文档**：
   - **根目录 `AGENTS.md`**：读取 `templates/agents.root.md.tpl` 生成全局技术栈、常用命令与全局红线。
   - **分层局部 `AGENTS.md`**：根据识别到的核心分层目录，读取 `templates/agents.scoped.md.tpl` 生成对应的分层隔离规则。
...
3. **AI-Native 即时生成（JIT）专属巡检脚本与 CI 门禁**：
   ...
   - 结合当前项目具体语言、Web 框架注解与模型目录特征，**JIT 编写生成专属的 `scripts/audit_doc_health.py`**（支持 `--strict` 门禁退出码）。
   - 为项目配置 Git Pre-commit（`.pre-commit-config.yaml`）与 CI 任务（如 Gradle `auditDocs` 任务 / GitHub Actions）。
4. **输出生成报告**：向用户展示生成的文档拓扑、架构风格自适应结果与物理门禁配置。
...
- 确定待修改的目标模块路径，自动向上递归解析所有适用的祖先 `AGENTS.md`，明确当前模块的局部不可违反红线。
...
### 步骤 3：ATDD 测试驱动与实现
- 先根据 Spec 编写接口测试/单元测试（Red 🔴）。
- 编写业务实现，确保严格遵守全局及局部 `AGENTS.md` 约束（Green 🟢）。
...
2. 检查 Root 与 Scoped `AGENTS.md` 是否完整，行数是否符合 Token 预算（Root $\le 100$, Scoped $\le 50$）。
